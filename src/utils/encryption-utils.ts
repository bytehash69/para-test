...
async function importSecretKey(keyString: string): Promise<CryptoKey> {
  try {
    ...
    throw new Error(
      `Failed to import secret key: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function encrypt(text: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(
      `Encryption process failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
export async function decrypt(encryptedText: string): Promise<string> {
  if (!encryptedText || !encryptedText.includes(":")) {
    throw new Error(
      "Encrypted text must be in the format 'IV(base64):Ciphertext(base64)'",
    );
  }
  ...
  if (parts.length !== 2 || !parts[0] || !parts[1]) {
    throw new Error(
      "Encrypted text format is invalid. Expected 'IV(base64):Ciphertext(base64)'.",
    );
  }
  ...
  try {
    ...
    iv = new Uint8Array(
      ivBuf.buffer.slice(ivBuf.byteOffset, ivBuf.byteOffset + ivBuf.byteLength),
    );
